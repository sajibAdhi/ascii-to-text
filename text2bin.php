		...
		<form action="" method="post">
			...
			<div class="row container">
				...
				<div class="col-sm-5 col-md-12 col-lg-5">
				    <h3><b>String to Binary</b></h3>
					<textarea class="" name="str" id="" cols="35" rows="10"><?php
						$bin=$_POST['bin'];
						$convert_to_string=$_POST['convert_to_string'];
						if(isset($convert_to_string)
						){
							if(count(explode(" ",$bin))>1
							){
								$arrayb=explode(" ",$bin);
							
								foreach($arrayb as $char){
									echo pack("C*",bindec($char));
								}
							}else{
								$len=strlen($bin)/8;
								for($i=0,$j=0;$i<$len;$i++,$j=$j+8){
									
									$arrayb=substr($bin,0+$j,8);
									echo pack("C*",bindec($arrayb));
								}
							}
						}
					?></textarea>
				</div>
				<div class="col-sm-2 col-md-12 col-lg-2 m-auto">
				    
					<input type="submit" value="Convert To Binary" name="convert_to_binary" />
					<input type="submit" value="Convert To String" name="convert_to_string" />
					<div class="form-check">
						<label class="form-check-label">
							<input type="checkbox" name="space" class="form-check-input" value="space">Space Added Binary
						</label>
					</div>
				</div>
				<div class="col-sm-5 col-md-12 col-lg-5">
				    <h3><b>Binary To String</b></h3>
					<textarea class="" name="bin" id="" cols="35" rows="10"><?php
							$str=$_POST['str'];
							$spaceb=$_POST['space'];
							$arrays=str_split($str);
							
							$convert_to_binary=$_POST['convert_to_binary'];
						if(isset($convert_to_binary) && isset($spaceb)
						){
							
							for($i=0;$i<strlen($str);$i++){
								if($i+1 < strlen($str)
								){
									echo str_pad(decbin(ord($arrays[$i])), 8, 0, STR_PAD_LEFT)." ";
								}else{
									echo str_pad(decbin(ord($arrays[$i])), 8, 0, STR_PAD_LEFT); 
								}
								
							}
						} elseif(isset($convert_to_binary)
							){
								for($i=0;$i<strlen($str);$i++){
									echo str_pad(decbin(ord($arrays[$i])), 8, 0, STR_PAD_LEFT);
							}
